...
public class MonthOverview implements Dialog {
    ...
    public Menu create(Player p, Object... args) {
        ...
        MenuBuilder builder = new MenuBuilder(Title.get("month-overview", "year", Integer.toString(year) , "month", Integer.toString(month)));
        ...
        for (Days d : Days.values()) {
            if (d.getNumber() <= maxDays) {
                if (monthObj.getDay(d.getNumber() - 1) != null) {
                    ...
                    if (monthObj.getDay(d.getNumber() - 1).hasPlayerClaimed(p.getUniqueId())) {
                        builder.addButton(new GUIButton(monthObj.getDay(d.getNumber() - 1).getClaimedIcon(), 1, Button.get("already-claimed", "day", Integer.toString(d.getNumber()))), d.getSlot());
                    } else if (day != d.getNumber()) {
                        builder.addButton(new GUIButton(monthObj.getDay(d.getNumber() -1 ).getLockedIcon(), 1, Button.get("locked-day", "day", Integer.toString(d.getNumber()))), d.getSlot());
                    } else if (day == d.getNumber()) {
                        builder.addButton(new GUIButton(Advent.getAvailableIcon(), 1, Button.get("claim-day", "day", Integer.toString(d.getNumber()))) {
                            @Override
                            public void onLeftClick(Player p) {
